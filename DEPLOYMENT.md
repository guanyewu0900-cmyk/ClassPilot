...
ClassPilot is a Node.js web app. It serves the teaching platform, uploaded files, bootstrap lesson data, and AI chat API from `server.js`.
...
Create `/etc/nginx/sites-available/classpilot`:
...
```bash
sudo ln -s /etc/nginx/sites-available/classpilot /etc/nginx/sites-enabled/classpilot
sudo nginx -t
