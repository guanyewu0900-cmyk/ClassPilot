...
function buildNewtonFaqKnowledge() {
  const sampleKnowledge = path.join(UPLOAD_DIR, "sample_knowledge_base.md");
  try {
    if (fs.existsSync(sampleKnowledge)) {
      return fs.readFileSync(sampleKnowledge, "utf8").trim();
    }
  } catch {}
  return "No sample knowledge base is available yet.";
}
...
function buildDemoBootstrap() {
  ...
  const promptConcept = "You are a friendly assistant helping Grade 8 students understand Newton’s first law. When a student asks a concept question, answer it directly and accurately in two or three sentences using simple language. Emphasize that motion continues without a net force when friction is absent. End your reply with one sentence that connects the concept to the experiment or activity they are about to do. Respond in the same language the student uses. Do not ask the student questions. Do not use markdown formatting, bullet points, or bold text. Do not give more than four sentences in total.";
...
async function callOpenAiCompatible(config, messages) {
  ...
  if (!upstream.ok) {
    const detail = data?.error?.message || data?.error || `HTTP ${upstream.status}`;
    throw new Error(`${config.label} returned an error: ${detail}`);
  }
  const answer = String(data?.choices?.[0]?.message?.content || "").trim();
  if (!answer) throw new Error(`${config.label} returned an empty answer`);
  return { answer, usage: data?.usage || null };
}

async function callAnthropic(config, systemText, question) {
  const requestBody = {
    model: config.model,
    max_tokens: 1200,
  ...
  const upstream = await fetch(joinApiUrl(config.baseUrl, "messages"), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${config.apiKey}`,
...
async function handleAiChat(req, res) {
  ...
  const modelKey = String(payload.model || "deepseek-chat").trim() || "deepseek-chat";
  const config = AI_MODELS[modelKey];
  if (!config) {
    sendJson(res, 400, { error: `Unsupported model: ${modelKey}` });
    return;
  }
  if (!String(config.apiKey || "").trim()) {
    sendJson(res, 503, { error: `${config.label} is not configured. Set ${config.keyName} in the environment or .env file.` });
    return;
  }
  if (!String(config.model || "").trim()) {
    sendJson(res, 503, { error: `${config.label} needs a model ID. Set ${config.modelName} in the environment or .env file.` });
    return;
  }
  ...
  try {
    const result = config.protocol === "anthropic"
      ? await callAnthropic(config, systemText, question)
    ...
    sendJson(res, 502, { error: err.message || "AI provider request failed" });
  }
}
