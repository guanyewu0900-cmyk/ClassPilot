...
const AI_MODEL_OPTIONS = [
  ...
  { value: "deepseek-reasoner", label: "DeepSeek Reasoner", hint: "Reasoning-oriented DeepSeek model" },
  { value: "openai", label: "ChatGPT / OpenAI", hint: "OpenAI model configured by the platform operator" },
  { value: "claude", label: "Claude / Anthropic", hint: "Anthropic model configured by the platform operator" },
  { value: "kimi", label: "Kimi / Moonshot AI", hint: "Kimi model configured by the platform operator" },
];
...
const LEGACY_DEMO_TEXT = new Map([
  ["牛顿第一定律实验教学示例", "Newton's First Law Experiment Teaching Demo"],
  ["导入PPT + 互动演示 + 课堂总结，并配置三位AI助教。", "Intro PPT + interactive demo + classroom summary, with three AI assistants configured."],
  ["课程导入PPT", "Course Introduction PPT"],
  ["互动演示", "Interactive Demo"],
  ["课堂总结PPT", "Class Summary PPT"],
  ["概念答疑助手", "Concept Q&A Assistant"],
  ["实验探究引导伙伴", "Experiment Inquiry Guide"],
  ["实验分析助手", "Experiment Analysis Assistant"],
]);

function englishDisplayText(value, fallback = "") {
  const text = String(value || fallback || "");
  return LEGACY_DEMO_TEXT.get(text) || text;
}

function englishCompileSummary(value) {
  let text = englishDisplayText(value);
  text = text.replace(/^已编译[:：]\s*(.+?)\s*（约\s*(\d+)\s*页）$/u, "Compiled: $1 (about $2 pages)");
  text = text.replace(/^已编译[:：]\s*(.+)$/u, "Compiled: $1");
  return text;
}
...
function normalizeModule(raw, fallbackId, index = 0) {
  ...
  compile.summary = englishCompileSummary(compile.summary);
  ...
  return {
    ...
    moduleName: englishDisplayText(raw?.moduleName, `Module ${index + 1}`),
    contentType,
    color,
    shape,
    positionX: px,
...
function normalizeProject(raw, index = 0) {
  ...
  return {
    id,
    name: englishDisplayText(raw?.name, `Sample Project ${index + 1}`),
    description: englishDisplayText(raw?.description),
    modules,
    connections,
...
function normalizeSnapshot(raw, index = 0) {
  ...
  return {
    projectId,
    publishId,
    name: englishDisplayText(raw?.name, `Sample Course ${index + 1}`),
    description: englishDisplayText(raw?.description),
